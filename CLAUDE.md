...
2. Set its `capabilities` honestly (e.g. `supportsRequestChanges: false` if it has no such verdict) — the UI adapts.
...
- `unreadBadge` always `nil` — no notifications-API integration yet.
...
Everything builds into `build/DerivedData` (so Sparkle's CLI tools are at `build/DerivedData/SourcePackages/artifacts/sparkle/Sparkle/bin`). Debug builds are unsigned and therefore **not sandboxed** — sandbox behavior only shows up in `make release-dry` builds; test there before shipping anything that touches files, network or Keychain.
